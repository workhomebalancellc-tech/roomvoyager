...
export default function ProfilePage() {
  ...
  // Fetch admin-created bookings from Firestore when user loads
  useEffect(() => {
    if (!user?.uid) return;
  ...
  const allBookings = [
    ...firestoreBookings.map(b => ({ ...b, _source: "admin" })),
    ...bookings.map(b => ({ ...b, _source: "local" })),
  ];
  const filteredBookings = bookingFilter === "all"
    ? allBookings
    : allBookings.filter(b => b.type === bookingFilter || b.status === bookingFilter || b.product === bookingFilter);
          ...
          <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.1)", borderRadius: "12px", padding: "4px", gap: "2px" }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                ...
                {t.icon} {t.label}
                {t.id === "bookings" && allBookings.length > 0 && (
                  <span style={{ marginLeft: "6px", background: ORANGE, color: "#fff", fontSize: "10px", fontWeight: "800", padding: "1px 6px", borderRadius: "999px" }}>{allBookings.length}</span>
                )}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: "860px", margin: "-40px auto 0", padding: "0 24px 80px" }}>
        ...
        {activeTab === "bookings" && (
          <div>
            ...
            {reviewOpen && (
              <div style={{ background: "#fff", borderRadius: "20px", boxShadow: "0 4px 24px rgba(0,59,149,0.1)", padding: "24px", marginBottom: "16px", border: `2px solid ${ORANGE}` }}>
                {reviewSent ? (
                  <div style={{ textAlign: "center", padding: "16px 0" }}>
                    <p style={{ fontSize: "36px", margin: "0 0 10px" }}>✅</p>
                    <p style={{ fontWeight: "800", color: "#111827", margin: "0 0 6px", fontSize: "16px" }}>Request sent!</p>
                    <p style={{ fontSize: "13px", color: "#6B7280", margin: "0 0 16px" }}>We'll review your booking within 1–2 business days and reach out to {user.email}.</p>
                    <button onClick={() => setReviewOpen(false)} style={{ background: NAVY, color: "#fff", border: "none", borderRadius: "8px", padding: "9px 20px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>Close</button>
                  </div>
                ) : (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                      <div>
                        <h3 style={{ fontSize: "16px", fontWeight: "800", color: "#111827", margin: "0 0 4px" }}>📋 Request Booking Confirmation Review</h3>
                        <p style={{ fontSize: "13px", color: "#6B7280", margin: 0 }}>Don't see your booking or points? Let us know and we'll look into it.</p>
                      </div>
                    ...
                    <form onSubmit={submitReviewRequest} style={{ display: "flex", flexDirection: "column", gap: "13px" }}>
                      ...
                      <div style={{ display: "flex", gap: "10px" }}>
                        <button type="submit" disabled={reviewLoading}
                          ...
                          {reviewLoading ? "Sending…" : "Submit Review Request →"}
                        </button>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {filteredBookings.map(b => {
                ...
                return (
                  <div key={b.id || b.reference} style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 2px 12px rgba(0,59,149,0.08)", overflow: "hidden", border: isAdmin ? `2px solid ${NAVY}` : "1px solid #E5E7EB" }}>
                    {/* Admin badge */}
                    {isAdmin && (
                      <div style={{ background: NAVY, padding: "5px 16px", display: "flex", gap: "6px", alignItems: "center" }}>
                        <span style={{ fontSize: "10px", color: "#93C5FD", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ Confirmed Booking</span>
                        {b.reference && <span style={{ fontSize: "10px", color: "#93C5FD" }}>· Ref: {b.reference}</span>}
                        {b.double && <span style={{ fontSize: "10px", color: ORANGE, fontWeight: "700" }}>🔥 Double Points</span>}
                      </div>
                    )}

                    {/* Card header */}
                    <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: "14px", cursor: "pointer" }} onClick={() => setExpandedBooking(isOpen ? null : (b.id || b.reference))}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "3px" }}>
                          <p style={{ fontWeight: "700", color: "#111827", fontSize: "15px", margin: 0 }}>{b.destination || "Your Booking"}</p>
                        ...
                        <p style={{ fontSize: "12px", color: "#6B7280", margin: 0 }}>
                          ...
                          {!isAdmin && b.reference ? ` · Ref: ${b.reference}` : ""}
                        </p>
                      </div>
                    ...
                    {isOpen && (
                      <div style={{ borderTop: "1px solid #F3F4F6", padding: "16px 20px", background: "#FAFAFA" }}>
                        ...
                        {/* Status controls — only for local bookings */}
                        {!isAdmin && (
